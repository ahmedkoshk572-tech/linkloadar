...
import { ActivityIndicator, Alert, AppState, FlatList, Image, Modal, Pressable, SafeAreaView, ScrollView, StyleSheet, Text, TextInput, View } from "react-native";
...
export default function App() {
  ...
  async function handleAnalyze() {
    ...
    setBusy(true); setPreview(null); setSelected(null); setMessage("شاهد الإعلان داخل التطبيق ثم اضغط X للمتابعة...");
  ...
  function closeAdAndContinue() {
    setAdVisible(false);
    setAdLoaded(false);
    const targetUrl = pendingAnalyzeUrl;
    setPendingAnalyzeUrl(null);
    if (targetUrl) void performAnalyze(targetUrl);
  }
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.app}><View style={styles.topbar}><View style={styles.brandRow}><View style={styles.logo}><Ionicons name="play" size={15} color="#fff" /></View><Text style={styles.brand}>Link<Text style={styles.brandAccent}>Load</Text></Text></View><View style={styles.statusPill}><View style={[styles.statusDot, apiReady === true && styles.statusReady]} /><Text style={styles.statusText}>{apiReady === true ? "متصل" : apiReady === false ? "غير متصل" : "..."}</Text></View></View>{tab === "home" ? <HomeScreen url={url} setUrl={setUrl} preview={preview} selected={selected} setSelected={setSelected} busy={busy} message={message} handleAnalyze={handleAnalyze} handleDownload={handleDownload} videoFormats={videoFormats} /> : tab === "history" ? <HistoryScreen downloads={downloads} share={share} /> : <SettingsScreen apiReady={apiReady} />}</View><AdModal visible={adVisible} loaded={adLoaded} onLoaded={() => setAdLoaded(true)} onClose={closeAdAndContinue} /><View style={styles.tabs}><TabButton icon="home-outline" active={tab === "home"} label="الرئيسية" onPress={() => setTab("home")} /><TabButton icon="download-outline" active={tab === "history"} label="التنزيلات" onPress={() => setTab("history")} badge={downloads.length || undefined} /><TabButton icon="settings-outline" active={tab === "settings"} label="الإعدادات" onPress={() => setTab("settings")} /></View></SafeAreaView>;
}

function AdModal({ visible, loaded, onLoaded, onClose }: { visible: boolean; loaded: boolean; onLoaded: () => void; onClose: () => void }) {
  return <Modal visible={visible} animationType="slide" presentationStyle="fullScreen" onRequestClose={onClose}>
    <SafeAreaView style={styles.adModalSafe}>
      <View style={styles.adModalHeader}>
        <View><Text style={styles.adModalEyebrow}>LinkLoad</Text><Text style={styles.adModalTitle}>إعلان</Text></View>
        <Pressable onPress={onClose} disabled={!loaded} style={({ pressed }) => [styles.adClose, !loaded && styles.adCloseDisabled, pressed && styles.pressed]} accessibilityLabel="إغلاق الإعلان"><Ionicons name="close" size={25} color="#fff" /></Pressable>
      </View>
      <View style={styles.adModalBody}>
        <WebView source={{ uri: ADSTERRA_SMARTLINK }} javaScriptEnabled domStorageEnabled originWhitelist={["http://*", "https://*"]} setSupportMultipleWindows={false} allowsBackForwardNavigationGestures={false} onLoadEnd={onLoaded} onShouldStartLoadWithRequest={(request) => request.url.startsWith("https://") || request.url.startsWith("http://")} startInLoadingState />
        {!loaded ? <View style={styles.adLoading}><ActivityIndicator color="#9b8fff" size="large" /><Text style={styles.adLoadingText}>جارٍ تحميل الإعلان...</Text></View> : null}
      </View>
      <Text style={styles.adModalHint}>{loaded ? "بعد انتهاء الإعلان اضغط X للعودة إلى LinkLoad." : "انتظر حتى يظهر الإعلان داخل التطبيق."}</Text>
    </SafeAreaView>
  </Modal>;
}
